...
export default function DietUserLike({ mealId, mealLike, mealViews }: Props) {
    ...
    const priorError = useSelector(
        (state: RootState) => state.profileSlice.PRIORERROR
    )
    const dispatch = useDispatch<AppDispatch>()

    const [localLike, setLocalLike] = useState<boolean>(
        userLikes.includes(mealId)
    )
    ...
    return (
        <div className="flex gap-5">
            <div
                className={`cursor-pointer my-2 flex items-center gap-2 bg-slate-100 w-fit px-2 py-1 rounded-xl hover:scale-110 duration-500 ${
                    status === 'pending' || loading ? 'animate-pulse' : ''
                }`}
            >
                {!error ? (
                    localLike ? (
