...
export default function DietSectionAnimation({
    diet,
    index,
}: DietSectionAnimationProps) {
    return (
        <motion.div
            key={index}
            whileInView="visible"
        ...
        >
            ...
            <h3 className="text-xl font-semibold text-center truncate text-slate-50">
                {diet.name}
            </h3>
            <div
                className={`${comfortaa.className} flex justify-between items-center text-lg text-slate-50 gap-3`}
            >
                ...
                <div className="flex">
                    <Link
                        href={`/diet/${diet.name.toLocaleLowerCase().replaceAll(' ', '-')}-${diet.id}`}
                    >
