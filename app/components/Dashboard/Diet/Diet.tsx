...
export default function DietDashboard() {
    ...
    const imageMap: { [key: string]: string } = mealData.reduce((map, item) => {
        ...
        <>
            <div className="m-5 p-5">
                ...
                <div className="flex flex-wrap gap-2">
                    {status === 'success' && userDiet?.map((data, idx) => (
                        <div key={idx} className="w-full xl:w-auto m-5 p-2 flex items-center bg-slate-50 shadow-md rounded-2xl overflow-hidden hover:scale-105 duration-700 cursor-pointer">
                            ...
                            <div className="p-4 flex-1">
                                ...
                                <p className="text-gray-600 mb-1"><strong>Calories:</strong> {data.calorie} kcal</p>
                                <p className="text-gray-600 mb-1"><strong>Protein:</strong> {data.protein} g</p>
                                <p className="text-gray-600 mb-1"><strong>Fat:</strong> {data.fat} g</p>
                                <p className="text-gray-600 mb-1"><strong>Carbs:</strong> {data.carb} g</p>
                                <p className="text-gray-600 mb-1"><strong>Date Added:</strong> {dateToday(data.id)}</p>

                            </div>
                        </div>
                    ))}

                    {status === 'pending' && Array.from({ length: 5 }).map((_, idx) => (
                        <div key={idx} className="xl:w-1/2 m-5 p-2 flex items-center bg-slate-50 shadow-md rounded-2xl overflow-hidden hover:scale-105 duration-700 cursor-pointer">
                            <div className="h-full overflow-hidden p-2">
                                <div className="bg-slate-300 h-full w-40 rounded-2xl animate-pulse">
                    ...
                    <div className="flex items-center justify-center  mt-40">
                        <div className="max-w-md mx-auto p-6 bg-slate-50 shadow-lg rounded-2xl text-center">
                            ...
                            <div className="text-gray-600 mb-6">
                                It seems like you haven't added any recent meal. Please add your recent diet to continue.
                            </div>
