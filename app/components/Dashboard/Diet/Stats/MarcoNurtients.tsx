...
export default function Macronutrients(props: { data: DataProps }) {
    ...
    useEffect(() => {
        ...
        if (startDate) {
            ...
            const sumFat = filteredIndices
                .reduce((sum, index) => sum + data.fat[index], 0);
            const sumCarb = filteredIndices
                .reduce((sum, index) => sum + data.carb[index], 0);
            const sumCalorie = filteredIndices
                .reduce((sum, index) => sum + data.calorie[index], 0);
            const sumProtein = filteredIndices
                .reduce((sum, index) => sum + data.protein[index], 0);
    ...
    return (
        <>
                ...
                <div className="mt-5 w-full px-4">
                    <div className="flex flex-col gap-7">
                        <div className="bg-slate-100 p-4 rounded-lg shadow-md">
                            <p className="text-gray-700 text-xl">Total Fat: <span className="font-semibold">{sumFat} grams</span></p>
                        </div>
                        <div className="bg-slate-100 p-4 rounded-lg shadow-md">
                            <p className="text-gray-700 text-xl">Total Carbs: <span className="font-semibold">{sumCarb} grams</span></p>
                        </div>
                        <div className="bg-slate-100 p-4 rounded-lg shadow-md">
                            <p className="text-gray-700 text-xl">Total Calories: <span className="font-semibold">{sumCalorie} kcal</span></p>
                        </div>
                        <div className="bg-slate-100 p-4 rounded-lg shadow-md">
                            <p className="text-gray-700 text-xl">Total Protein: <span className="font-semibold">{sumProtein} grams</span></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-center items-center xl:w-1/2">
                <MarcoNutDoughnut nutritionalData={{ sumFat, sumCarb, sumCalorie, sumProtein }} />
            </div>
        </>
    );
};
