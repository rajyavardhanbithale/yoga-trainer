...
import {
    setAmbientMusic,
    setAmbientMusicName,
} from '@/lib/store/practice/audioSlice'
            ...
            {ambientMusic && (
                <DropdownMenu open={open} onOpenChange={setOpen}>
                    <DropdownMenuTrigger asChild>
                        <span className="cursor-pointer bg-slate-200 font-semibold px-2 py-0.5 rounded-2xl mt-2">
                            Select Music
                        </span>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="w-56 bg-blue-950 text-slate-50 border-none">
                        ...
                        {availableAmbientMusic.map((music, idx) => (
                            <DropdownMenuItem
                                ...
                                onClick={() =>
                                    dispatch(setAmbientMusicName(music))
                                }
                            >
                                {music}
                            </DropdownMenuItem>
