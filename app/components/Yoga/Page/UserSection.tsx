...
export default function UserSection() {
    ...
    useEffect(() => {
        dispatch(setPoseData(poseInfo.filter(pose => pose.id === Number(id))[0]))
    }, [id])
    ...
    return (
        <>
            {data &&
                <div className="flex flex-col justify-center items-center mb-2">
                    <div className="flex gap-3  text-slate-800 px-5 py-1 rounded-2xl">
                        <span className="text-3xl capitalize font-bold">{data.name}</span>
                        <span className="text-3xl capitalize font-bold">-</span>
                        <span className="text-3xl capitalize font-bold">{data.originalName}</span>
                    </div>
            ...
            <div className="grid grid-cols-12 gap-5">
                <div className="col-span-6 h-[50vh] bg-red-500">
